# coding: utf-8
class ChartsBurndown3Controller < ChartsController
  ...
  def get_data_for_burndown_chart
    ...
    issues.each do |issue|
      ...
      range_diff_days = (range_end_date - issue_start_date) #←オリジナル。BUGかなーと思ってissue_start_dateをrange_start_dateに変更
    ...
    @range[:keys].each_with_index do |key,index|
      ...
      issues.each do |issue|
        ...
        if issue.tracker.is_in_roadmap?
          total_base_estimated_hours[index] += base_estimate #見積ラインは見積工数をそのまま利用
          total_estimate_hours[index] += estimated #進捗率は実績から求めた予測工数を利用
